...
# Define the exercise list with load attributions
exercises = {
    "Chest": [
    ...
    "Calves": [
        ...
        {"name": "Tibialis Raise", "equipment": ["bodyweight"], "difficulty": "beginner", "cues": "Heels against wall, raise toes.", "muscles": {"calves": 1.0}},  # Anterior tibialis
...
def generate_markdown():
    with open("exercise_library.md", "w") as f:
        f.write("# Exercise Library List\\n\\n")
        f.write("A comprehensive list of exercises covering 10 major muscle groups, with exactly 20 exercises each, including precise volume load attribution.\\n\\n")
        for group, group_exercises in exercises.items():
            f.write(f"## {group}\\n")
            f.write("| Exercise | Difficulty | Primary Muscles (Volume Load) |\\n")
            f.write("| --- | --- | --- |\\n")
            for ex in group_exercises:
                muscles_str = ", ".join([f"{k} ({(v*100):.0f}%)" for k, v in ex["muscles"].items()])
                f.write(f"| {ex['name']} | {ex['difficulty'].capitalize()} | {muscles_str} |\\n")
            f.write("\\n")

def generate_sql():
    with open("supabase/seed_exercises_extended.sql", "w") as f:
        f.write("-- Extended Exercise Library Seed (20 per muscle group)\\n")
        f.write("insert into public.exercises (name, category, equipment, difficulty, description, setup_cues, execution_cues, primary_muscles, is_system)\\n")
        f.write("values\\n")
        
        values_lines = []
        for group, group_exercises in exercises.items():
            for ex in group_exercises:
                # Convert muscles dict to array of objects
                muscles_json = [{"muscle": k, "contribution": v} for k, v in ex["muscles"].items()]
                muscles_str = json.dumps(muscles_json).replace("'", "''")
                equipment_arr = "array[" + ",".join([f"'{eq}'" for eq in ex["equipment"]]) + "]"
                
                line = f"  ('{ex['name'].replace(\"'\", \"''\")}', '{group.lower()}', {equipment_arr}, '{ex['difficulty']}', 'Targeting {group}.', 'Setup carefully.', '{ex['cues'].replace(\"'\", \"''\")}', '{muscles_str}'::jsonb, true)"
